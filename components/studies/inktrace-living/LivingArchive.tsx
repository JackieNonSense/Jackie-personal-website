...
export default function LivingArchive({still=false}:{still?:boolean}){
 ...
 const archiveWindow=state.open?<motion.div className={`${w.window} ${s.window}`} data-living-window data-scene={state.scene} data-mode={current.mode} data-paused={held} initial={quiet?false:{opacity:0,y:6}} animate={{opacity:1,y:0}} transition={{duration:quiet?0:DUR.base,ease:EASE}} onKeyDown={event=>{if(event.key==='Escape'){event.stopPropagation();close();}}}>
  ...
  <div className={w.demoLabel}><span>FEATURE DEMO / SAMPLE CONTENT</span><span>{pending?'Loading scene…':`${String(SCENES.findIndex(d=>d.id===state.scene)+1).padStart(2,'0')} / ${sceneName.toUpperCase()}`}</span></div>
  <motion.div key={state.scene} role="tabpanel" id={`panel-${state.scene}`} aria-labelledby={`tab-${state.scene}`} className={s.scenePanel} initial={quiet||held?false:{opacity:.4,x:6}} animate={{opacity:1,x:0}} transition={{duration:quiet||held?0:DUR.slow,ease:EASE}}>
 ...
 return <main className={`${w.study} ${s.archive}`}>
  <aside className={w.intro}>
   <a className={w.back} href="/studies/inktrace/living-archive/review">← Art direction</a><h1 data-archive-title>INKTRACE</h1>
   <div className={w.introCopy}><span className={w.kicker}>THE LIVING ARCHIVE</span><h2>A world,<br/>in the<br/>making.</h2><p>I build complex worlds. Their chapters, people, timelines and notes shouldn’t live in disconnected tools.</p><p>InkTrace brings those pieces into one writing workspace.</p><motion.a {...feedback} className={w.visit} href="https://inktrace.app" target="_blank" rel="noopener noreferrer">EXPLORE INKTRACE <span>↗</span></motion.a></div>
