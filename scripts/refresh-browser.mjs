...
export async function browser(existing){
 ...
 let id=0;const pending=new Map(),errors=[],listeners=new Set();
 ws.addEventListener('message',e=>{const m=JSON.parse(e.data);if(m.method==='Runtime.exceptionThrown')errors.push(m.params.exceptionDetails);if(m.method)listeners.forEach(listener=>listener(m));const p=pending.get(m.id);if(p){clearTimeout(p.timer);pending.delete(m.id);m.error?p.reject(Error(JSON.stringify(m.error))):p.resolve(m.result);}});
 ...
 return{send,run,until,delay,shot,errors,onEvent:listener=>{listeners.add(listener);return()=>listeners.delete(listener);},close:async()=>{ws.close();if(!existing)await fetch('http://127.0.0.1:9333/json/close/'+target.id,{signal:AbortSignal.timeout(5000)}).catch(()=>{});}};
}
